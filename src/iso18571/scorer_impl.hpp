...
std::pair<double, double> magnitude_error_from_state(
    const ArrayView& x,
    const ArrayView& y,
...
std::pair<double, double> magnitude_error_impl(
    const ArrayView& x,
    const ArrayView& y,
    double window_size
) {
...
template <typename Series>
PhaseResult phase_candidate_for_shift(
    const Series& reference,
    const Series& comparison,
    Index reference_start,
    Index comparison_start,
    ...
    result.correlation.rho_e = correlation_for_shift(
        reference,
        comparison,
        reference_start,
        comparison_start,
...
PhaseResult phase_candidate_from_correlation(
    Index reference_start,
    Index comparison_start,
    Index length,
    Index n_eps,
...
template <typename Series>
std::pair<PhaseResult, PhaseResult> dual_phase_candidates_for_shift(
    const Series& reference,
    const Series& comparison,
    const PhaseCache& cache,
    Index shift,
...
PhaseResult compute_phase_alignment(const Series& reference, const Series& comparison, const ScoreParams& params) {
    ...
    if (result.alignment.length < 9) {
        ...
        append_warning(
            result.diagnostics,
            DiagnosticComponent::Phase,
            DiagnosticCode::PhaseShiftClampedToUnshifted
        );
    }
...
double phase_score(const CurveView& reference, const ScoreParams& params, const PhaseAlignment& alignment) {
    ...
    return std::pow(
        (max_allowable_time_shift_threshold - std::abs(static_cast<double>(alignment.n_eps))) /
            max_allowable_time_shift_threshold,
        static_cast<double>(params.k_p)
    );
}

MagnitudeResult magnitude_score_from_values(
...
SlopeResult fused_slope_score_from_values(
    const ArrayView& reference_values,
    const ArrayView& comparison_values,
    const ScoreParams& params
) {
    ...
    if (denominator == 0.0) {
        ...
        append_warning(
            result.diagnostics,
            DiagnosticComponent::Slope,
            DiagnosticCode::SlopeZeroReferenceDenominator
        );
        return result;
    }
...
ScoreResult score_components_impl(const CurveView& reference, const CurveView& comparison, const ScoreParams& params) {
    ...
    const ArrayView comparison_values = value_view_from_curve(
        comparison,
        result.phase.alignment.comparison_start,
        result.phase.alignment.length
    );
    const ArrayView reference_values = value_view_from_curve(
        reference,
        result.phase.alignment.reference_start,
        result.phase.alignment.length
    );
    ...
    result.overall =
        params.w_z * result.corridor.score +
        params.w_p * result.phase.score +
        params.w_m * result.magnitude.score +
        params.w_s * result.slope.score;
    return result;
}

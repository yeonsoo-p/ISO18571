...
template<typename T>
void c2c_power_of_two (std::complex<T>* data, std::size_t length, bool forward, T fct) {
    if (length == 0)
        return;
    if ((length & (length - 1U)) != 0)
        throw std::invalid_argument("complex FFT length must be a power of two");
    static_assert(sizeof(std::complex<T>) == sizeof(detail::cmplx<T>),
                  "std::complex storage is incompatible with the local FFT representation");
    detail::cfftp<T> plan(length);
    plan.exec(reinterpret_cast<detail::cmplx<T>*>(data), fct, forward);
}

...
#if defined(_MSC_VER) && (defined(_M_X64) || defined(_M_AMD64))
void cpuid(int leaf, int subleaf, int (&registers)[4]) {
...
bool supports_v2() {
    ...
    return sse3 && ssse3 && sse41 && sse42 && popcnt && cx16;
#else
    return false;
#endif
}
...
bool supports_v3() {
    ...
    return supports_v2() && osxsave && os_supports_avx() && avx && avx2 && bmi && bmi2 && fma && lzcnt &&
        movbe;
#elif defined(_MSC_VER) && (defined(_M_X64) || defined(_M_AMD64))
    int registers[4] = {};
    cpuid(1, 0, registers);
    ...
    const bool lzcnt = (registers[2] & (1 << 5)) != 0;
    return supports_v2() && osxsave && os_supports_avx() && avx && avx2 && bmi && bmi2 && fma && lzcnt && movbe;
#else
    return false;
#endif
}
